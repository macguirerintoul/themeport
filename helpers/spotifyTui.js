...
module.exports = {
	...
	formatForSpotifyTui: function (scheme) {
		...
		formattedScheme.theme.brightBlack = convert.hex.rgb(
			scheme.ansi.brightBlack
		);
		formattedScheme.theme.brightRed = convert.hex.rgb(scheme.ansi.brightRed);
		formattedScheme.theme.brightGreen = convert.hex.rgb(
			scheme.ansi.brightGreen
		);
		formattedScheme.theme.brightYellow = convert.hex.rgb(
			scheme.ansi.brightYellow
		);
		formattedScheme.theme.brightBlue = convert.hex.rgb(scheme.ansi.brightBlue);
		formattedScheme.theme.brightMagenta = convert.hex.rgb(
			scheme.ansi.brightMagenta
		);
		formattedScheme.theme.brightCyan = convert.hex.rgb(scheme.ansi.brightCyan);
		formattedScheme.theme.brightWhite = convert.hex.rgb(
			scheme.ansi.brightWhite
		);
